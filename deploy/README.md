...
One release-ordering requirement comes with that lockstep. The chart's default
shape binds :80, which needs the `cap_net_bind_service` file capability the
image gained in the commit that made `hostNetwork` the default — so the `v*`
release cut for a given chart version has to be built from that commit or later.
An older image under this chart crash-loops with `Permission denied`, and says
so; the check under [Binding :80](#binding-80) tells you which you have.
...
| [`baremetal-hostnetwork`](provider/baremetal-hostnetwork/values.yaml) | none; `hostNetwork` DaemonSet on :80/:443 — **the chart's default shape** |
...
so a preset that sets `--proxy-protocol` and an `--engine uring` in
`controller.extraArgs` work together.
...
**This is the chart's default**, so a `helm install` with no values file at all
gets it; the preset exists to say so explicitly and to be the source of the
rendered manifest. The ports belong to the node, so a second copy on the same
...
with an address to read. Set `controller.publishAddress`
