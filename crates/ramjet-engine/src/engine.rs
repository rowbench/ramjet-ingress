...
impl Engine {
    ...
    pub fn bind(
        ...
        let (listeners, http_addr) = bind_intake(config.http, cores, config.backlog)?;
    ...
    pub fn run(self) -> io::Result<()> {
        ...
        // thread accepts and deals descriptors out.
        let mut acceptor: Option<AcceptorThread> = None;
        let mut intakes = Vec::with_capacity(cores);
        if listeners.len() == cores {
            for listener in listeners {
                intakes.push(Intake::Listener(listener.into_raw_fd()));
            }
        } else {
            ...
            for channel in theirs {
                intakes.push(Intake::Channel(channel.into_raw_fd()));
            }
            let listener = listeners
                .into_iter()
                .next()
                .ok_or_else(|| io::Error::other("no listener was bound"))?;
            acceptor = Some(AcceptorThread::start(
                listener,
                mine,
                Arc::clone(&shutdown),
            )?);
        ...
        for (core, (intake, notify)) in intakes.into_iter().zip(notifies).enumerate() {
...
impl AcceptorThread {
    fn start(
        listener: OwnedFd,
        channels: Vec<OwnedFd>,
        shutdown: Arc<AtomicBool>,
    ) -> io::Result<AcceptorThread> {
        let handle = thread::Builder::new()
            .name("ramjet-uring-accept".to_owned())
            .spawn(move || accept_loop(listener, channels, shutdown))?;
...
fn accept_loop(listener: OwnedFd, channels: Vec<OwnedFd>, shutdown: Arc<AtomicBool>) {
    let mut next = 0usize;
    while !shutdown.load(Ordering::Relaxed) {
        ...
        loop {
            match accept(listener.as_raw_fd()) {
                Ok(Some(fd)) => {
                    let target = next % channels.len();
                    next = next.wrapping_add(1);
                    // Four bytes into a socket pair whose buffer starts in the
                    // kilobytes: this cannot actually block.
                    if sys::write(channels[target].as_raw_fd(), &fd.to_le_bytes()).is_err() {
                        // SAFETY: the core never received this descriptor, so
                        // nobody else owns it.
                        unsafe { sys::close(fd) };
                    }
                }
...
mod tests {
    ...
    fn binding_port_zero_reports_the_port_the_kernel_chose() {
        ...
        assert_ne!(engine.http_addr().port(), 0, "the port must be resolved");
